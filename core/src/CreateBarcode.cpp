...
static SymbologyIdentifier SymbologyIdentifierZint2ZXing(const CreatorOptions& opts, const ByteArray& ba, std::string_view &eanAddOn)
{
	...
	if (format & (EAN8 | EAN13 | UPCA | UPCE)) {
		...
		if (format & (Aztec | Code128 | Code16K | DotCode))
			ret.modifier = '1';
...
#undef X
		...
		if (auto val = ecLevel(); val && !(format() & AllLinear))
		...
		if (auto val = height(); val && (format() & AllLinear))
			zint->height = *val;
		else
...
Barcode CreateBarcode(const void* data, int size, int mode, const CreatorOptions& opts)
{
	...
	zint->input_mode = mode == -1 || mode == UNICODE_MODE
						? opts.gs1() && (opts.format() & BarcodeFormat::AllGS1) ? GS1_MODE : UNICODE_MODE : mode;
	...
	if ((format & (BarcodeFormat::MicroQRCode | BarcodeFormat::QRCode | BarcodeFormat::RMQRCode))) {
	...
	} else if (format & BarcodeFormat::EANUPC) {
