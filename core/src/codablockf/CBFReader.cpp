...
BarcodeData DetectSymbol(const BinaryBitmap& image)
{
	...
	for (int rowNumber = 0; rowNumber < image.height(); rowNumber++) {
		...
		for (;;) {
			if (!next.skipSymbol()) {
				return MatrixBarcode(DecoderResult(FormatError("Skip fail")), DetectorResult{}, BarcodeFormat::CodablockF);
			}
			...
			if (code == CODE_STOP) {
				...
				if (!DetectStopCode(next)) {
					return MatrixBarcode(DecoderResult(FormatError("Stop terminator fail")), DetectorResult{}, BarcodeFormat::CodablockF);
				}
		...
		if (rows.empty()) {
			...
			if (Size(rows.front()) != Size(rawCodes)) {
				return MatrixBarcode(DecoderResult(FormatError("Bad row size")), DetectorResult{}, BarcodeFormat::CodablockF);
			}
			rows.push_back(rawCodes);
		}
	...
	if (Size(rows) < 2) {
		return MatrixBarcode(DecoderResult(FormatError("< 2 rows")), DetectorResult{}, BarcodeFormat::CodablockF);
	}

	// Check row indicators, first row is total no. of rows
	if (rows[0][1] == CODE_CODE_C) {
		if (rows[0][2] + 2 != Size(rows)) {
			return MatrixBarcode(DecoderResult(FormatError("Bad row indicator total")), DetectorResult{}, BarcodeFormat::CodablockF);
		}
	} else {
	...
	for (int i = 1; i < Size(rows); i++) {
		if (rows[i][1] == CODE_CODE_C) {
			if (rows[i][2] - 42 != i) {
				return MatrixBarcode(DecoderResult(FormatError("Bad row indicator index")), DetectorResult{}, BarcodeFormat::CodablockF);
			}
		} else {
			if (rows[i][2] >= 26) {
				if (rows[i][2] - 26 + 6 != i) {
	...
	for (int i = 0; i < Size(rows); i++) {
		...
		for (int j = 3; j < Size(row) - (i + 1 == Size(rows) ? 2 : 0); j++) {
			if (!rowText.decode(row[j])) {
				return MatrixBarcode(DecoderResult(FormatError("Decode")), DetectorResult{}, BarcodeFormat::CodablockF);
			}
		}
		if (i == 0) {
	...
	return MatrixBarcode(std::move(decoderResult), DetectorResult({}, Position(tl, tr, br, bl)), BarcodeFormat::CodablockF);
}
