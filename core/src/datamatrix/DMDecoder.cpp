...
DecoderResult Decode(ByteArray&& bytes, const std::string& characterSet, const bool isDMRE)
{
	...
	try {
		while (!done && bits.available() >= 8) {
			...
			switch (oneByte) {
			case 0: Diagnostics::put("ASCError(0)"); return DecodeStatus::FormatError;
			...
			case 233: // Structured Append
				...
				firstFNC1Position = 5;
				Diagnostics::fmt("SA(%d,%d,%s)", sai.index, sai.count, sai.id.c_str());
				break;
			case 234: // Reader Programming
				...
				readerInit = true;
				break;
			case 235: Diagnostics::put("UpSh"); upperShift.set = true; break; // Upper Shift (shift to Extended ASCII)
