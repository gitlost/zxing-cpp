...
std::vector<DataBlock> GetDataBlocks(const ByteArray& rawCodewords, const int version, const int ecLevel)
{
	...
	static const unsigned short int table[] = {
    ...
    for (int i = 0; i < 3 && table[tablePos + (3 * i)]; i++) {
		...
		for (int j = 0; j < batchSize; j++) {
			//printf("%s(%d) %s: block %d, i %d, batchSize %d, dataLength %d, eccLength %d, cw %d\n", __FILE__, __LINE__, __func__, block + j, i, batchSize, dataLength, eccLength, cw);
			result[block + j].numDataCodewords = dataLength;
			auto& blockCodewords = result[block + j].codewords;
			blockCodewords.resize(dataLength + eccLength, 0);
			for (int k = 0; k < dataLength + eccLength; k++) {
				blockCodewords[k] = rawCodewords[cw++];
			}
		}
		block += batchSize;
    }
