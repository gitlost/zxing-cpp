...
static Clusters FindCandidates(const BitMatrix& image, bool tryHarder, bool isPure, bool reversed)
{
	...
	for (int y = margin; y < height - margin; y += skip) {
		...
		(std::tie(next, idx) = FindLRAP(next), next.isValid()) {
			...
			if (p != PointI{}) {
#define ZXING_SUPPORT_MINIMAL_SIZE_MIRCOPDF417
...
static BarcodeData ScanCandidate(const BitMatrix& image, const Cluster& lraps, const bool isPure)
{
	BitMatrixModuleCursorF startCur(image, centered(lraps.front()), PointF(lraps.back() - lraps.front()), lraps.front().width / 27.f);
	startCur.turnLeft();
	startCur.step(isPure ? -0.5 : -1);
...
BarcodesData Reader::read(const BinaryBitmap& image, int maxSymbols) const
{
	...
	for (int rotate90 = 0; rotate90 <= static_cast<int>(tryRotate); ++rotate90) {
		...
		for (bool reversed : {false, true}) {
			for (const auto& x : FindCandidates(*binImg, _opts.tryHarder(), _opts.isPure(), reversed)) {
				auto v = ScanCandidate(*binImg, x, _opts.isPure());
				if (rotate90)
