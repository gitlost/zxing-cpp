...
int main(int argc, char* argv[])
{
	...
	for (std::string filePath : filePaths) {
		...
		if (!meta.getNString(ResultMetadata::SYMBOLOGY_IDENTIFIER).empty()) {
			std::cout << "Identifier: \"" << meta.getNString(ResultMetadata::SYMBOLOGY_IDENTIFIER) << "\"\n";
		}
		...
		if (meta.getInt(ResultMetadata::STRUCTURED_APPEND_CODE_COUNT) >= 1) {
			std::cout << "Structured Append\n"
					  << "  Sequence: " << meta.getInt(ResultMetadata::STRUCTURED_APPEND_SEQUENCE) << "\n"
					  << "  Total:    " << meta.getInt(ResultMetadata::STRUCTURED_APPEND_CODE_COUNT) << "\n"
					  << "  ID:       " << meta.getNString(ResultMetadata::STRUCTURED_APPEND_ID) << "\n"
					  << "  ECI:      " << meta.getInt(ResultMetadata::STRUCTURED_APPEND_ECI) << "\n";
		}
		...
		if (meta.getBool(ResultMetadata::READER_INIT)) {
			std::cout << "Reader Initialisation\n";
		}

		if (hints.enableDiagnostics()) {
			std::cout << "Diagnostics";
			auto diagnostics = meta.getStringList(ResultMetadata::DIAGNOSTICS);
