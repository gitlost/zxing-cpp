...
"X XX  X  X X X XX  X X X  X XX X  X  X XX \n";
	...
	}
	{
		...
		auto cOpts = CreatorOptions(format);
		Barcode barcode = CreateBarcodeFromText("1234", cOpts);
		check(__LINE__, barcode, "]XF", "77-2", "37 37 2D 32", false, "]XF\\00002677-2", "5D 58 46 37 37 2D 32", "77-2", "Text",
			  "0x0 22x0 22x5 0x5");
#ifdef ZXING_READERS
		auto rOpts = ReaderOptions().setFormats(format).setIsPure(true);
		auto wOpts = WriterOptions().addQuietZones(false);
	...
	}
	{
		...
		check(__LINE__, barcode, "]XF", "77-2/62A", "37 37 2D 32 2F 36 32 41", false, "]XF\\00002677-2/62A",
...
TEST(CreateBarcodeTest, ZintEANUPCAddOn)
{
	{
		BarcodeFormat format = BarcodeFormat::EAN8;
...
TEST(CreateBarcodeTest, ZintISO8859_1)
{
	...
	}
	{
		...
		auto wOpts = WriterOptions().addQuietZones(false);
		wOpts.scale(2).addHRT(true); // Scale 2 to get HRT to appear
		Image image = WriteBarcodeToImage(barcode, wOpts);
		std::string imageStr = getImageStr(image);
...
"                                                                      X                                                                 \n";
	...
	}
	{
		...
		auto cOpts = CreatorOptions(format, "eci=3");
		Barcode barcode = CreateBarcodeFromText("1234é", cOpts);
...
TEST(CreateBarcodeTest, ZintBinary)
{
	...
	}
	{
		...
		auto cOpts = CreatorOptions(format, "eci=BINARY");
		std::string data("\x00\x5C\x7E\x7F\x80\x81\xC0\xFF", 8);
		Barcode barcode = CreateBarcodeFromBytes(data, cOpts);
		check(__LINE__, barcode, "]C0",
			  std::string("\x00\x5C\x7E\x7F\xC2\x80\xC2\x81\xC3\x80\xC3\xBF", 12) /*TextMode::Plain - UTF-8*/,
...
TEST(CreateBarcodeTest, ZintECI)
{
	{
...
{
	{
		...
		auto cOpts = CreatorOptions(format, "GS1");
