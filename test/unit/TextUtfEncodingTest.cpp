...
{
	using namespace ZXing::TextUtfEncoding;
	auto escape = [](const wchar_t* str) { return EscapeNonGraphical(str); };
	...
	EXPECT_EQ(escape(L"\xD800Z"), "<0xED><0xA0><0x80>Z"); // Unpaired high surrogate
	EXPECT_EQ(escape(L"A\xDC00"), "A<0xED><0xB0><0x80>"); // Unpaired low surrogate
}
