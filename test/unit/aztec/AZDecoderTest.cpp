...
DecoderResult Decode(const BitArray& bits, const CharacterSet optionsCharset = CharacterSet::Unknown);
...
TEST(AZDecoderTest, SymbologyIdentifier)
{
	...
	check_si(__LINE__, getData("11101111010001000101000000000000000000000000010101111100100001000100011"), "]zA", "2001", 0, 4);

	// ECI 3 with Structured Append (no ID) with AIM
	check_si(__LINE__, getData("111011110100010001010000000000001010100010000000000000000011"), "]zB", "AB", 0, 4);
}
