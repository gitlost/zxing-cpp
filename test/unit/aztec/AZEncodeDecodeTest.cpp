...
namespace {
	...
	void TestEncodeDecode(const std::string& data, bool compact, int layers) {
		...
		DecoderResult res =
			Aztec::Decoder::Decode({{aztec.matrix.copy(), {}}, aztec.compact, aztec.codeWords, aztec.layers}, false);
		...
		res = Aztec::Decoder::Decode({{std::move(matrix), {}}, aztec.compact, aztec.codeWords, aztec.layers}, false);
		ASSERT_EQ(res.isValid(), true);
		EXPECT_EQ(data, res.text());
	}
	...
	void TestWriter(const std::wstring& data, CharacterSet charset, int eccPercent, bool compact, int layers) {
		...
		DecoderResult res = Aztec::Decoder::Decode({{matrix.copy(), {}}, aztec.compact, aztec.codeWords, aztec.layers}, false);
