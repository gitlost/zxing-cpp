...
namespace ZXing {
	namespace QRCode {
		DecoderResult DecodeBitStream(ByteArray&& bytes, const Version& version, ErrorCorrectionLevel ecLevel, const std::string& hintedCharset, Diagnostics& diagnostics);
	}
}
...
TEST(QRDecodedBitStreamParserTest, SimpleByteMode)
{
    ...
    auto result = DecodeBitStream(ba.toBytes(), *Version::VersionForNumber(1), ErrorCorrectionLevel::Medium, "", diagnostics).text();
